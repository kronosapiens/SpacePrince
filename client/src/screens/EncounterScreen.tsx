...
export default function EncounterScreen() {
  const [profile, setProfile] = useLocalStorage<Profile>(
    "space-prince-profile",
    DEFAULT_PROFILE
  );
  const [run, setRun] = useLocalStorage<RunState | null>(
    "space-prince-run",
    null
  );
  ...
  const getAfflictionLevel = (value: number) =>
    Math.min(3, Math.floor(value / 3));
          ...
          <div className="panel-header">
            <div>
              ...
              <p className="encounter-hint">
                Hover a planet to inspect. Click a planet to act.
              </p>
            </div>
            <div className="log-meta">
              <div className="tag">
                Distance | {frozenDistance ?? roundDisplay(run?.score ?? 0)}
              </div>
            </div>
          </div>
              ...
              <TurnTrack
                ...
                actionSlot={
                  encounter.completed && !run?.over ? (
                    <button
                      className="btn turn-continue-inline"
              ...
              <div className="chart-visuals">
                ...
                <div>
                  <ChartVisual
                    ...
                    showAspects={Boolean(
                      hoveredOpponent ?? actionOpponent ?? hoveredPlanet
                    )}
