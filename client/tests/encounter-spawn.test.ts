...
describe("afflictedSideState — opponent spawn affliction (MECHANICS §11)", () => {
  it("draws each fielded planet from the tiers, and reaches all of them", () => {
    ...
    for (let seed = 1; seed <= 50; seed++) {
      const state = afflictedSideState(seededChart(seed), [...PLANETS], mulberry32(seed));
  ...
  it("reserves at least one 12-point step below Resolve at spawn", () => {
    for (let seed = 1; seed <= 50; seed++) {
      const chart = seededChart(seed, "t");
      const state = afflictedSideState(seededChart(seed), [...PLANETS], mulberry32(seed));
      for (const p of PLANETS) {
        expect(state[p].affliction).toBeLessThanOrEqual(combustionCeiling(chart.planets[p]) - 12);
      }
    }
  });

  it("seeds only the fielded roster", () => {
    const state = afflictedSideState(seededChart(7), ["Moon"], mulberry32(7));
