...
- **One spacing unit on the encounter.** `--gap` on `.combat` is both the border around the composition and the gutter between the wheels, and the wheel is sized from the viewport with no maximum (`spec/design/SCREENS.md §3.1.1`).
