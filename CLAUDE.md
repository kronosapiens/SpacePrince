...
- **The encounter declares one spacing value.** `--pad` on `.combat` is the border and the only spacing — no grid gap, since each wheel already holds 2% of its own box clear — and the wheel is sized from the viewport with no maximum (`spec/design/SCREENS.md §3.1.1`).
