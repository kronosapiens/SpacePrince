...
- [x] **Combat voice moments** (`PLANETS.md §1` secondary usages).
...
- [x] **Transits.**
  Compute the current sky from wall-clock time via the existing ephemeris path (`client/src/astronomy/compute.ts`); ghost glyphs ride the rim on Title and Chart Study, and a transit conjoining or opposing a natal placement sits a shade brighter.
  Fragment-mood and ambient-tint bias were left out of v1 — the ring carries the layer; revisit if it wants more presence.
...
- [x] **Lifetime layers** (`SCREENS.md §7.4`).
  Fragment archive shipped: every fragment heard across the Prince's life, derived from runs' `seenFragmentIds` (nothing new stored), browsable in Chart Study with attributions.
  The houses-encountered wedge flourish is deferred — the chart draws no house wedges yet, so that layer needs its ground first (see Deferred).
- [x] **Achievements v1** (`MECHANICS.md §11.2`, bitmap reserved in `STATE.md`).
