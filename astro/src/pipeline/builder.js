 ...
 * @param {{ getLongitude: (planet: import('../types.js').PlanetId, unixMs: number) => number }} opts.referenceProvider
...
export function buildLongitudeArchive({
  ...
  if (!referenceProvider || typeof referenceProvider.getLongitude !== "function") {
    throw new Error("referenceProvider.getLongitude is required");
  }
  ...
  for (const planet of PLANETS) {
    ...
    while (blockStart < rangeEndUnixMs) {
      ...
      const midLongitude = referenceProvider.getLongitude(planet, mid);

      const coeffs = fitChebyshev(cfg.order, (u) => {
        const unixMs = blockStart + ((u + 1) / 2) * (blockEnd - blockStart);
        const rawLongitude = referenceProvider.getLongitude(planet, unixMs);
...
export function createMockReferenceProvider() {
  return {
