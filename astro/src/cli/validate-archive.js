...
function usage() {
  ...
  console.log(
    [
      ...
      "  --out path                 Output report path, default out/validate.report.json",
      "  --step-minutes N           Validation cadence, default 15",
      "",
      "Example:",
      "  npm run validate:archive -- --archive out/archive.json --out out/archive.report.json --step-minutes 30",
    ].join("\n")
  );
}
...
async function main() {
  ...
  const outPath = getStringArg(args, "out", "out/validate.report.json");
